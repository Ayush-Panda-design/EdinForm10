...
- ✅ **httpOnly cookie auth** — `setAuthCookie()` called on sign-in/sign-up, `clearAuthCookie()` on sign-out; `createContext` reads token from `req.cookies.edinform_token` as fallback to `Authorization` header
...
### Database
- ✅ **Full Drizzle migration written** — `0001_edinform_full_schema.sql` creates all 13 tables with enums, indexes, foreign keys, and partial unique indexes; migration journal updated
