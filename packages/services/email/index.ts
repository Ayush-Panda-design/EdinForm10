...
const DEFAULT_FROM = process.env.EMAIL_FROM ?? "EdinForm <noreply@edinform.io>";
...
export class EmailService {
  ...
  async sendWelcomeEmail(to: string, fullName: string): Promise<EmailResult> {
    return this.send({
      to,
      subject: "Welcome to EdinForm 🎉",
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:auto">
          <h1 style="color:#6366f1">Welcome to EdinForm, ${fullName}!</h1>
          ...
          <p style="color:#888;font-size:12px;margin-top:32px">EdinForm — The modern form builder</p>
        </div>
      `,
    });
  }
  ...
  async sendEmailVerification(opts: {
    ...
    return this.send({
      to: opts.to,
      subject: "Verify your EdinForm email address",
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:auto">
          ...
          <p style="color:#888;font-size:12px;margin-top:16px">If you didn't create a EdinForm account, you can safely ignore this email.</p>
        </div>
      `,
    });
  }
  ...
  async sendPasswordReset(opts: {
    ...
    return this.send({
      to: opts.to,
      subject: "Reset your EdinForm password",
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:auto">
    ...
    return this.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:auto">
          ...
          <p style="color:#888;font-size:12px;margin-top:32px">Powered by EdinForm</p>
        </div>
      `,
    });
  }
