...
app.use(
  helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc: ["'self'"],
        scriptSrc: ["'self'", "'unsafe-inline'"], // Scalar UI needs inline scripts
    ...
    crossOriginEmbedderPolicy: false, // needed for Scalar API docs
  })
);
...
// Health
// ---------------------------------------------------------------------------
app.get("/", (_req, res) => res.json({ message: "EdinForm API is running 🚀" }));
