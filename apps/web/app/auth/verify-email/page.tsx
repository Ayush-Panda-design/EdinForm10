  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 p-4">
      <div className="text-center max-w-sm">
        {status === "pending" && (
          <>
            <Loader2 className="w-10 h-10 animate-spin text-stone-600 mx-auto mb-4" />
        ...
        {status === "success" && (
          <>
            ...
            <Link href="/dashboard" className="inline-block px-6 py-2.5 rounded-xl bg-stone-600 text-white font-semibold hover:bg-stone-700 transition-colors">
              Go to dashboard
            </Link>
          </>
        )}
        {status === "error" && (
          <>
            ...
            <p className="text-gray-500 mb-6">{message}</p>
            <Link href="/auth/login" className="text-stone-600 hover:text-stone-700 font-medium">Sign in</Link>
          </>
        )}
      </div>
    </div>
